...
class Blockchain(object):
    ...
    difficulty_target = "0000"  # For simplicity, the difficulty level is set at 4 zeros.
    ...
    def __init__(self):
        '''
        Store the entire blockchain as a list
        '''
        self.nodes = set()
    ...
    def hash_block(self, block):
        '''
        method encodes a block into array of bytes and then hashes it
        the dictionary must be sorted to avoid inconsistent hashes later on
        '''
    ...
    def valid_proof(self, index, hash_of_previous_block, transactions, nonce):
        ...
        '''
        # create a string containing the hash of the previous block and
        # the block content, including the nonce
        content = f'{index}{hash_of_previous_block}{transactions}{nonce}'.encode()

        print(f'Nonce: {nonce} - Hash:{hashlib.sha256(content).hexdigest()}')
    ...
    def valid_chain(self, chain):
        ...
        .- verifies that the hash of each block is correctly recorded in the next block
        .- verifies that the nonce in each block is valid.
        '''
    ...
    def update_blockchain(self):
        '''
        Checking that the blockchain from neighboring nodes is valid and
        that the node with the longest valid chain is the authoritative one;
        if another node with a valid blockchain is longer than the current one,
...
def new_transaction():
    '''
    route to add transactions to the current block:
    '''
...
def mine_block():
    '''
    route to allow miners to mine a block so that it can be added to the blockchain
    '''
...
def add_nodes():
    '''
    route allows a node to register one or more neighboring nodes
    '''
...
def sync():
    '''
    route allows a node to synchronize its blockchain with its neighboring nodes
    '''
